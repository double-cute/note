...
| [N -> N](#七n---n) | **只有间接** | set(table), key, many-to-many | 少用（**N - N属于复杂关系，复杂关系只能用连接表间接连接，包括N <-> N**） |
| [1 <-> N]() | [直接]() | set(inverse), one-to-many  /  many-to-one | 少用 |
| | 间接连接 | set(inverse), many-to-many(unique)  /  join, many-to-one | **常用** |
| [N <-> N]() | [间接]() | set(table), many-to-many | 少用 |
| [1 <-> 1]() | [直接]() | one-to-one(property-ref)  /  many-to-one(unique) | 尽量不用 |
| | [主键关联]() | generator(foreign), one-to-one  /  one-to-one | 尽量不用 |
