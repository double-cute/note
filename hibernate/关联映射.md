...
## 目录
1. [关联关系的种类]()
2. [关联是否需要连接表？（直接连接和间接连接）]()
3. [Hibernate对关联关系的表达：端口和关联映射]()
4. 关联关系：

| 关联关系 | 关联方式 | 使用标签 | 关注 |
| --- | --- | --- | --- |
| 1 -> N | [直接]() | many-to-one | **最常用** |
| | [间接]() | join, many-to-one | 很少用 |
| 1 -> 1 | [直接]() | many-to-one | 很少用 |
| | [间接]() | join, many-to-one | 很少用 |
| | [**主键关联**]() | **generator(foreign)**, **one-to-one** | 尽量不用 |
| 1 -> N | [直接]() | set, key, **one-to-many** | 不用 |
| | [间接]() | set(**table**), key, **many-to-many(unique)** | **常用** |
| N -> N | [**只有间接**]() | set(table), key, many-to-many | 少用 |
