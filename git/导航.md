...
[**2. &nbsp&nbspgit仓库的结构：git rev-parse**](配置/git仓库的结构：git%20rev-parse.md#git仓库的结构git-rev-parse)
...
**3. &nbsp&nbsp仓库的初始化：git init**

> 由于仓库是从无到有的创建，因此git init命令可以在非工作区使用.
>
>> 使用它肯定是因为还没有工作区.

| 命令：**仓库初始化** | 说明 |
| --- | --- |
| git init | 以当前目录为**工作区根目录**创建**.git/** |
| git init **指定目录** | 以指定目录作为**工作区根目录**创建**.git/**<br>如果指定目录不存在则创建它 |
