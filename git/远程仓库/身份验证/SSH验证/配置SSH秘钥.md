...
1. [SSH简介](#一ssh简介)
2. [SSH通信基本模型](#二ssh通信基本模型)
3. [生成密钥对：ssh-keygen](#三生成密钥对ssh-keygen--)
4. [密钥文件的内容](#四密钥文件的内容)
5. [交换公钥](#五交换公钥)
6. [GitHub多账户管理](#六github多账户管理)：
  1. [SSH公钥的主机唯一性](#611-ssh公钥的主机唯一性)
  2. [主机如何快速确定一个公钥是否已经存在：公钥指纹](#612-主机如何快速确定一个公钥是否已经存在公钥指纹--)
  3. [SSH通信协议下GitHub服务器如何验证客户端的身份？](#62-ssh通信协议下github服务器如何验证客户端的身份--)
  4. [本地git客户端如何配置SSH秘钥？](#63-本地git客户端如何配置ssh秘钥--)
7. [总结：SSH URL的真正含义](#七总结ssh-url的真正含义--)
...
#### 6.1.2 主机如何快速确定一个公钥是否已经存在：公钥指纹  [·](#目录)
...
#### 6.2 SSH通信协议下GitHub服务器如何验证客户端的身份？  [·](#目录)
...
### 七、总结：SSH URL的真正含义  [·](#目录)

- 一般情况下：回话发起端使用的SSH通信软件名称@主机别名:远程资源的路径/资源
  - 例如：winscp@company_terminal:myaccount/code/src/proj1/index.html

<br>

- 在使用git客户端和GitHub通信的情况下：git@主机别名（一般取为GitHub账号名）:GitHub账号名/托管仓库.git
  - 例如：git@myacc:myacc/test.git
  - 这样SSH通信软件会使用myacc别名所持有的私钥生成数字签名并进行接下来的通信解密工作.
